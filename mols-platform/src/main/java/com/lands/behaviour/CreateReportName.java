...
public class CreateReportName implements NodeServicePolicies.OnCreateNodePolicy,NodeServicePolicies.OnUpdatePropertiesPolicy {
    ...
    private Logger logger = Logger.getLogger(CreateReportName.class);

    public void init() {

        if (logger.isDebugEnabled()) logger.debug("Initializing create name behaviors");
        // Create behaviours
    ...
    public void createNameForNode(NodeRef nodeRef){
        ...
        if(nodeService.exists(nodeRef)){
            ...
            String approval_date  = alfrescoformatDatetoString((Date)nodeService.getProperty(nodeRef,QName.createQName(ValuationModel.NAMESPACE_VALUATION_MODEL, ValuationModel.PROP_APPROVAL_DATE)));
            String subject = (String)nodeService.getProperty(nodeRef,QName.createQName(ValuationModel.NAMESPACE_VALUATION_MODEL, ValuationModel.PROP_SUBJECT));
            String agency = (String)nodeService.getProperty(nodeRef,QName.createQName(ValuationModel.NAMESPACE_VALUATION_MODEL, ValuationModel.PROP_AGENCY));
            String report_name = agency + ValuationModel.EXTRA_DIVIDER + subject + ValuationModel.EXTRA_DIVIDER + approval_date;
