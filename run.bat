...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f mols-share
	call %MVN_EXEC% clean package -pl mols-share,mols-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill mols-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f mols-acs
	call %MVN_EXEC% clean package -pl mols-integration-tests,mols-platform,mols-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl mols-platform,mols-integration-tests,mols-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl mols-platform,mols-integration-tests
EXIT /B 0
:purge
